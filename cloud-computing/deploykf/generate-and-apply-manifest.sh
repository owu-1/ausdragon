...
export deploykf_subdomain_name="$deploykf_subdomain_name"
...
yq '.metadata.annotations."nginx.ingress.kubernetes.io/proxy-ssl-name" = strenv(deploykf_subdomain_name) |
    .metadata.annotations."external-dns.alpha.kubernetes.io/hostname" = "*." + strenv(deploykf_subdomain_name) + ", " + strenv(deploykf_subdomain_name) |
    .spec.rules[0].host = strenv(deploykf_subdomain_name) |
    .spec.rules[0].http.paths[0].backend.service.port.number = env(deploykf_https_port) |
    .spec.rules[1].host = "*." + strenv(deploykf_subdomain_name) |
...
yq '.deploykf_core.deploykf_istio_gateway.gateway.hostname = strenv(deploykf_subdomain_name) |
